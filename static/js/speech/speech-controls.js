...
function barKeepIntro() {
  if (step === 0) {
    ...
    document.getElementById("characterSpeech").innerText =
      barkeep_speech.intro2;
    step++;
  } else if (step === 1) {
    document.getElementById("characterSpeech").innerText =
      barkeep_speech.intro3;
    step++;
  } else {
...
export function setSpeech() {
  ...
  if (page === "cast-sending-stone") {
    document.getElementById("characterSpeech").innerText =
      barkeep_speech.sendingStone;
  } else if (page === "tavern") {
    tavernChat(adventurer);
  } else {
    document.getElementById("characterSpeech").innerText =
      adventurer === STEPHANIE ? stephanie_speech[page] : louise_speech[page];
  }
}
